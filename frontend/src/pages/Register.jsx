    ...
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        if (success && token) {
          ...
          setApiError(message || 'Đăng nhập Google thất bại')
        }
      } catch (err) {
        const msg = err.response?.data?.message || err.response?.data?.Message
        setApiError(msg || 'Đăng nhập Google thất bại. Vui lòng thử lại.')
      } finally {
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-dark overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          >
            ...
            <span className="text-white text-sm font-semibold">Đăng ký với Google</span>
          </button>

          <p className="text-center text-sm text-white/40 mt-8">
            Đã có tài khoản?{' '}
            <Link to="/dang-nhap" className="text-primary font-medium transition-all duration-300 hover:text-primary-dark hover:-translate-y-[1px] hover:scale-[1.02] inline-block">
              Đăng nhập
            </Link>
          </p>
        </div>
